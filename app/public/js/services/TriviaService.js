    ...
    return $http ({
      ...
      for(let i=0; i < 5; i++){
        ...
        for (let a of response.data.results[i].incorrect_answers) {
          ...
          answers.push({
            answer: a.replace(/&quot;/gi, '"').replace(/&#039;/gi, "'").replace(/&/gi, '').replace(/acute;/gi,'').replace(/shy;/gi, '').replace(/ldquo;/gi,'"').replace(/rsquo;/gi,"'").replace(/hellip;rdquo;/gi,"..."),
            correct: false
          });
        }
        ...
        answers.push({
          answer: response.data.results[i].correct_answer.replace(/&quot;/gi, '"').replace(/&#039;/gi, "'").replace(/&/gi, '').replace(/acute;/gi,'').replace(/shy;/gi, '').replace(/ldquo;/gi,'"').replace(/rsquo;/gi,"'").replace(/hellip;rdquo;/gi,"..."),
          correct: true
        });
        ...
        questionsAndAnswers.push({
          question: response.data.results[i].question.replace(/&quot;/gi, '"').replace(/&#039;/gi, "'").replace(/&/gi, '').replace(/acute;/gi,'').replace(/shy;/gi, '').replace(/ldquo;/gi,'"').replace(/rsquo;/gi,"'").replace(/hellip;rdquo;/gi,"..."),
          answers: answers
        });
      }
